...
class GEOAuditor:
    ...
    def analyze_page(self, page: Dict[str, Any]) -> Dict[str, Any]:
        ...
        checks_passed["structured_format"] = has_question_headings or has_enough_structure
        ...
        for s in schema_data:
            if isinstance(s, dict) and (s.get("datePublished") or s.get("dateModified")):
                has_date = True
                break
        ...
        for check_name, passed in checks_passed.items():
            if not passed:
                ...
                issue = {
                    ...
                    "fix_suggestion": _get_fix_suggestion(check_name, page, blocked_crawlers),
                    "estimated_impact": "",
                }
                issues.append(issue)
