...
class AnalyzerAgent(Agent):
    ...
    async def run(self) -> AgentResult:
        ...
        max_pages = int((self.project_config.adapter_config or {}).get("max_pages", 15))
        ...
        try:
            ...
            try:
                ...
                logger.info(
                    f"[analyzer] Canonical engine: {len(canonical_issues)} issues"
                )
            ...
            try:
                ...
                logger.warning(
                    f"[analyzer] Soft-404 detection failed (non-fatal): {exc}"
                )
            ...
            try:
                ...
                try:
                    ...
                    logger.debug(
                        f"[analyzer] robots.txt fetch failed (non-fatal): {exc}"
                    )
            ...
            try:
                ...
                logger.warning(
                    f"[analyzer] Topical authority failed (non-fatal): {exc}"
                )
            ...
            try:
                ...
                logger.warning(
                    f"[analyzer] Duplicate content failed (non-fatal): {exc}"
                )
            ...
            try:
                ...
                async with _httpx.AsyncClient(
                    ...
                    robots_result = await rs_auditor.fetch_robots(
                        domain, client=_rs_client
                    )
                    ...
                    sitemap_result = await rs_auditor.fetch_sitemap(
                        sitemap_url, client=_rs_client
                    )
            ...
            try:
                ...
                if gsc_keywords:
                    ...
                    logger.info(
                        ...
                        + (
                            f" (skipped: {intent_geo_result.skipped_reason})"
                            if intent_geo_result.skipped_reason
                            else ""
                        )
                    )
                else:
                    logger.info(
                        "[analyzer] Intent/GEO: no GSC keywords available, skipped"
                    )
            except Exception as exc:
                logger.warning(
            ...
            if p.robots_meta and "noindex" in p.robots_meta.lower():
                ...
                issues.append(
                    _issue(
                        ...
                        + (" (likely intentional)" if is_legal else ""),
            ...
            if p.images_without_alt > 0:
                issues.append(
                    _issue(
                        "accessibility",
                        "images_without_alt",
                        "medium" if p.images_without_alt > 3 else "low",
                        p.url,
    ...
    def _check_schema(self, pages: List[PageData], domain: str) -> List[Dict[str, Any]]:
        ...
        for p in pages:
            ...
            if (
                is_home
                and "Organization" not in p.schema_types
                and "WebSite" not in p.schema_types
            ):
                issues.append(
                    _issue(
                        "schema",
    ...
    def _check_security(self, pages: List[PageData]) -> List[Dict[str, Any]]:
        ...
        for p in pages:
            ...
            missing = [
                h for h in REQUIRED_SECURITY_HEADERS if h not in p.security_headers
            ]
            if missing:
                issues.append(
                    _issue(
    ...
    def _check_core_web_vitals(
        self, psi_results: List[PageSpeedResult]
    ) -> List[Dict[str, Any]]:
