...
class SEOIssue(Base):
    ...
    # Auto-Fix Apply Tracking (Welle 2)
    fix_applied_at = Column(DateTime)  # set when ApplyAgent successfully applied a fix
    applied_by = Column(String(50))  # 'claude_auto' / 'manual' / 'rolled_back'
    git_commit_hash = Column(String(64))  # SHA des Fix-Commits
    fix_diff = Column(Text)  # unified diff
    fix_error = Column(Text)  # bei fail: stacktrace
